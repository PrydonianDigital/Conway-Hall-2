<?php
	...
	function page_meta() {
		...
		$cmb_page = new_cmb2_box( array(
			'id'			=> 'page',
			'title'			=> 'Page Side Images',
			'object_types'	=> array( 'page' ),
		) );
		$cmb_page->add_field( array(
			'name'	   => 'Show Images',
			'id'		 => $prefix . 'showimg',
			'description'	=> __( 'check this to show side images for the page', 'ch' ),
			'type'	   => 'checkbox',
		) );
		$page_images = $cmb_page->add_field( array(
			'id'			=> 'sideimages',
			'type'			=> 'group',
				'options'		=> array(
				'group_title'   => __( 'Image {#}', 'ch' ),
				'add_button'	=> __( 'Add Another Image', 'ch' ),
				'remove_button' => __( 'Remove Image', 'ch' ),
				'sortable'	  => true, // beta
			),
		) );
		$cmb_page->add_group_field( $page_images, array(
			'name'			=> 'Image',
			'id'			=> 'sideimg',
			'type'			=> 'file',
		) );
	...
	add_action( 'cmb2_admin_init', 'sundayconcert_meta' );
	function sundayconcert_meta() {
		$prefix = '_sundayconcert_';
		$cmb_page = new_cmb2_box( array(
			'id'			=> 'sundayconcert',
			'title'			=> 'Sunday Concert Pre-Concert Talks/Recitals',
			'object_types'	=> array( 'tribe_events' ),
		) );
		$cmb_page->add_field( array(
			'name'	   => 'Show Images',
			'id'		 => $prefix . 'description',
			'description'	=> __( 'Add info about Pre-Concert Talks/Recitals here', 'ch' ),
			'type'	   => 'wysiwyg',
		) );
	}