...
(function($) {
	...
	$.fn.planit = function(options) {
		...
		var defaults = {
				basepath: '/wp-content/themes/conway-hall/planit/',
				roomName: 'room-name',
		...
		function loadConfig(callback) {
			var xmlPath = settings.basepath + '/' + settings.roomName + '/xml/config.xml';
			...
			$.get(xmlPath, function(xmldata) {
				...
				$(xmldata).find('layout').each(function() {
					...
					imgUrls.push(settings.basepath + '/' + settings.roomName + '/floorplans/' + $(this).attr("title") + '.' + $(xmldata).find('layoutFileType').text());
		...
		function getPlanitLeft() {
			...
			if (settings.showOptions) {
				...
				$.each(settings.options, function(key, value) {
					...
					pLeft += '<a id="' + value.id + '" href="#" data-alt="' + value.id + '_mob"><div><i class="icon-' + value.id + '" aria-hidden="true"></i></div><span>' + value.name + '</span></a>';
					counter += 1;
					if (counter % 2 === 0) pLeft += ' </li>';
				});
		...
		function getPlanitRight() {
			...
			var pRight = '<div id="planit_right"><div id="main_content_panel"><div id="preloader"><div></div></div>' +
					   	 ...
					   	 '<img id="image_floorplan_base" src="' + settings.basepath + '/' + settings.roomName + '/floorplans/base.jpg" />' +
					   	 '<img id="current_layer" src="' + settings.basepath + '/' + settings.roomName + '/floorplans/' + currentLayout + '.' + settings.layoutFileType + '" /><div id="marker"></div>';

			// Overlay images
			$.each(settings.options, function(key, value) {
				pRight += '<img id="image_' + value.id + '" class="overlays" src ="' + settings.basepath + '/' + settings.roomName + '/floorplans/' + value.id + '.svg" />';
			});
		...
		function getLightbox() {
			var lightbox = '<div id="lightbox" style="position:relative;height:'+floorplanHeight+';">' +
						   '<img id="lightbox_main_image" src="' + settings.basepath + '/' + settings.roomName + '/floorplans/base.jpg" style="width: 100%; position: relative" />' +
						   '<img id="lightbox_current_layer" src="' + settings.basepath + '/' + settings.roomName + '/floorplans/' + currentLayout + '.' + settings.layoutFileType + '" style="position:absolute; left:0; top: 0; z-index:2;" />';

			$.each(settings.options, function(key, value) {
				lightbox += '<img id="lightbox_' + value.id + '" src ="' + settings.basepath + '/' + settings.roomName + '/floorplans/' + value.id + '.' + settings.layoutFileType + '" style="position:absolute; display:none; left:0; top: 0; z-index:3;" />';
			});
		...
		function getGallery() {
			...
			$.each(settings.gallery, function(key, value) {
				...
				gallery += '<a href="' + settings.basepath + '/' + settings.roomName + '/gallery/main/' + value.filename + '" data-current-gallery-image="' + (key+1) + '"><img title="' + value.title + '" class="left" src="' + settings.basepath + '/' + settings.roomName + '/gallery/thumbs/' + value.filename + '" width="70" height="70" style="' + (key === 0 ? "opacity:0.5" : "") + '" alt="" /></a>';
				if (key % 2 !== 0) gallery += ' </li>';
			});
		...
		function setLayout(layout) {
			...
			var src = settings.basepath + "/" + settings.roomName + "/floorplans/" + layout + "." + settings.layoutFileType;
		...
		$("ul#capacities li").live({
			...
			click: function(e) {
				...
				$("a#single_image").attr('href', settings.basepath + '/' + settings.roomName + '/floorplans/' + currentLayout + '.' + settings.layoutFileType);
			}
		});
			...
			$('#panaround_virtual_tour').fadeIn(function() {
				setTimeout(function() {
					...
					embedpano({
						...
						xml: settings.basepath + "/" + settings.roomName + "/tour/tour.xml",
						target: "html5_tour",
						html5: "auto",
			...
			showMenu('gallery', function() {
				if (isMobile) {
					setTimeout(function() { // allow time for the height transition to play out
						setGalleryImage(settings.basepath + '/' + settings.roomName + '/gallery/main/image-1.jpg');
					},450);
				} else {
					setGalleryImage(settings.basepath + '/' + settings.roomName + '/gallery/main/image-1.jpg');
				}
			...
			if (isMobile) {
				...
				if (direction == 'left') {
					if (settings.gallery.length > (currentImageArrNumber+1)) {
						...
						$('#secondary_title > span').html(selectedGalleryImage.title);
						setGalleryImage(settings.basepath + '/' + settings.roomName + '/gallery/main/' + nextImage.filename);
				...
				if (direction == 'right') {
					if ((currentImageArrNumber-1) >= 0) {
						...
						$('#secondary_title > span').html(selectedGalleryImage.title);
						setGalleryImage(settings.basepath + '/' + settings.roomName + '/gallery/main/' + previousImage.filename);
